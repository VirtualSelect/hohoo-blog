...
function renderFeed(items, siteUrl, locale) {
  const en=locale==='en';const language=en?'en':'zh';
  const base=siteUrl.replace(/\/$/,'');const prefix=en?'/en':'';
  const entries=[...items].sort((a,b)=>b.publishedAt.localeCompare(a.publishedAt)).slice(0,100);
  return '<?xml version="1.0" encoding="UTF-8"?>\n<rss version="2.0" xmlns:atom="http://www.w3.org/2005/Atom"><channel>'+
    ...
    entries.map(item=>{
      ...
      const summary=translated?.summary ?? item.originalSummary ?? item.summary;
...
module.exports=function newsFeed(context){return {
  name:'news-feed',
  async postBuild({outDir}) {
    const items=JSON.parse(await fs.readFile(path.join(context.siteDir,'data/news/items.json'),'utf8'));
    await fs.mkdir(path.join(outDir,'news'),{recursive:true});
    await fs.writeFile(path.join(outDir,'news/rss.xml'),renderFeed(items,context.siteConfig.url,context.i18n.currentLocale));
  },
};};
