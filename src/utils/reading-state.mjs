...
export function parseReadingState(raw) {
  ...
  const ids = list => [...new Set(Array.isArray(list) ? list.filter(id => typeof id === 'string' && /^(?:[a-f0-9]{20}|paper:\d{4}\.\d{4,5})$/.test(id)) : [])];
  return {saved:ids(value?.saved),read:ids(value?.read)};
}
export function toggleReadingState(state, field, id) {
  if (!['saved','read'].includes(field) || !/^(?:[a-f0-9]{20}|paper:\d{4}\.\d{4,5})$/.test(id)) return state;
