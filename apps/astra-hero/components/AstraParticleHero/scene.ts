...
export async function createAstraScene(canvas: HTMLCanvasElement, section: HTMLElement, signal: AbortSignal,
  onStatus: (status: SceneStatus) => void): Promise<AstraScene> {
  const path = await createPathTexture('/hohoo.svg', signal);
  ...
  function resize() {
    ...
    sectionTravel = Math.max(1, sectionRect.height - height - 64);
