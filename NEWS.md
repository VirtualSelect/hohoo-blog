...
- 来源：Hugging Face、Google DeepMind、OpenAI。
- 每天北京时间约 08:35 运行一次（GitHub 调度可能延迟）。
- 最多 5 条/UTC 自然日，每个来源最多 2 条；同日重复运行仍受总量限制。
...
发送内容仅为公开资讯的标题与短摘录。每批至多 5 次摘要调用，每次最多请求 450 个输出 token。此限制不等同于费用上限，费用取决于服务计价。当前不自动重试模型请求，以免重复计费；调用失败、配置不完整、输出格式或长度不符合要求时保留来源摘录，并写入回退记录。

摘要仅基于 RSS 提供的信息，不会读取原文补充事实。AI 输出不保证准确，需人工审核。中文标题保存在 `titleZh`；原始标题始终保留。
...
- 英文界面保持英文标签；启用中文摘要时，英文页面也会展示同一份中文摘要。没有自动生成双语摘要。
