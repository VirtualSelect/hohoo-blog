...
每两小时执行一次，计划在北京时间每天 00:35、02:35、04:35、06:35、08:35、10:35、12:35、14:35、16:35、18:35、20:35、22:35（GitHub 调度可能延迟）：RSS → 研究相关性筛选 → 去重与限量 → 数据校验 → 三语构建 → 自动提交默认分支。多次运行共用每日额度，不按每次运行增加额度。规则检查不是独立事实核验，摘要保留来源归属。自动翻译暂时关闭，不需要模型密钥。

Actions 摘要显示运行时间、各来源读取状态、筛选前候选数、相关性拒绝数与最终新增数。没有新增不等于没有执行；先检查当天运行记录，再查看摘要和报告。每天多次调度提供补偿机会，但不保证准点启动。

每天合计最多 10 条，AIHOT 最多 6 条，其他来源各最多 2 条；每日额度按 UTC 日期计算（北京时间 08:00 重置）。仅考虑近 14 天且日期有效的条目。通过技术与主题门槛后，优先选择 RAG、结构化输出、工具调用、MCP、上下文工程、评测、可观测性、模型机制、机器人控制与仿真等重点内容；同等贴合度下按来源优先级（AIHOT 优先）、发布时间和稳定 ID 排序。该排序是编辑规则，不是论文质量或事实核验评分，不对已经入库的内容做替换，也无法预知当天晚些时候的候选。AIHOT 是聚合来源，条目链接通往其阅读页与原始出处；日期是 Feed 提供的日期。相同事件不同链接仍可能未被识别。
