  ...
  for (const minutes of [0, -1, "15", NaN]) {
    assert.throws(
      () =>
        collectEntries(
          content([
            doc("first-call", "2026-09-11", { reading_minutes: minutes }),
          ]),
        ),
      /reading_minutes/,
    );
