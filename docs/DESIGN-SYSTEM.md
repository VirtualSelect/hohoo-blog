...
实践页的本地教学工具复用浅蓝、淡紫和杏色语义 token，以细侧线、编号与折叠控件区分。展开后桌面左右分栏，768px 以下单列；控件至少 44px 触摸区域，原生键盘操作。保持无持续动效、无新增图形库。首页仅增加低权重入口，不占据首屏。
...
## 2026-09-20 本地视觉精修（待审阅）

当前分支保留暖白与深绿强调；夜间底色采用中性炭黑，内容面使用灰绿。精选入口采用同高主次操作链接，文章列表只突出一篇真实内容；首页指南针按需展开。示意图只承担内容导览，不代表实际架构或实验指标。实现及验证记录见 reports/EDITORIAL-POLISH-20260920.md，尚未合并发布。

### 柔和辅助色（本地待审阅）

保留绿色主要操作与中性底色，增加雾蓝、淡紫、暖杏三组 ink / soft 变量；夜间独立调整，禁止直接使用浅色背景值。辅助色用于示意图、项目封面、方向编号、资讯细线和正文引用，不表示完成度或可信度。正文继续使用统一文本色，颜色之外保留文字标签。

### 方向与项目色的作用域

使用现有 domain 作为学习配色依据：ai-apps 绿、llm 雾蓝、embodied-ai 淡紫。首页精选、文章精选、专题和路线延续同一规则。项目封面使用独立 project-tone，不代表领域或完成状态。说明块必须保留概念 / 注意事项等文字标签；交互统一 visible focus、选中描边及减少动效回退。

### 内容呈现与阅读增强（本地待审阅）

- 首页项目采用交错双栏，手机按内容顺序单列；文章摘句必须来自真实文章并保留跳转链接。底部三个出口连接文章、项目和关于页。
- 项目封面可以采用排版和线稿，但不得冒充截图。博客真实界面仅在打开原生 dialog 后加载，关闭即卸载；Java 记录明确标注历史摘录，不能伪装成实时 API 调用。
- 示意图及已有项目截图复用 ExpandableFigure，支持 Escape、焦点恢复、主题与小屏幕。页面没有真实截图时，不填充假素材。
- 正文宽屏是读者本机偏好；代码超过 12 行才加入展开按钮，没有 JavaScript 时仍可阅读完整代码。手机阅读工具采用正文内 sticky，避免遮挡全站浮层。
- 实践对照必须区分简化代码、实际观察和结论边界；错误复盘使用错误、调整、再观察三段，不能把重试成功解释成原因已证实。
- RSS、随笔封面、关于页工作方式分别使用轻量雾蓝/暖杏/淡紫面；其余正文保持中性底色，避免所有区域都变成卡片。
- Radar 同一天只在时间轴显示日期；较长来源摘要可展开，已读仅弱化摘要，来源和操作保持可读。

### 实践手册（2026-09-22）

首页封面、教程与项目使用同一实践手册组件。雾蓝承载入口与阶段，暖杏承载观察和自检，代码保留中性阅读底色；颜色不代表验证成功或正确性。阶段控件采用可聚焦按钮及 aria-pressed，移动端代码与证据上下排列。真实记录与简化片段必须分开标注；进阶练习默认收起，原文不依赖展开操作阅读。

### 对话记忆实验室（2026-09-22）

在现有 Java 项目和教程中使用同一工作台，不增加一级导航。雾蓝选择轮次，淡紫区分助手历史，暖杏承载真实观察；回放和裁剪模式明确分开。裁剪不显示原有模型回答或 Token，所有新控件保留 44px 目标和可见焦点。原始中文记录加 lang 标记，说明与控件使用三语言；手机将请求与观察上下排列。

### 问题驱动工作台

实践页使用标题、双栏细线目录与单个打开的实验面板。蓝色标记应用，淡紫标记模型理解，暖杏标记具身；颜色不暗示实验成功。手机目录和状态对照单列，控件至少 44px，打开面板可聚焦，关闭返回入口。只挂载当前工具，退出后重置；不新增持续动画或依赖。
